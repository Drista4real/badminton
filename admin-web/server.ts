...
try {
  if (!admin.apps.length) {
    ...
    if (projectId && clientEmail && privateKey) {
      ...
      console.log('✅ Firebase Admin initialized successfully (PORT 3000)');
    } else {
...
async function startServer() {
  ...
  app.get("/api/data-health", (req, res) => {
    res.json({ status: "mega-ok-3000", firebaseDb: !!db, userCacheSize: cacheUsers.length });
  });
  ...
  // Helper to convert any Firestore Timestamp safe to client-side json structure {_seconds, _nanoseconds}
  ...
  app.post("/api/generate-qr", async (req, res) => {
    ...
    try {
      ...
      if (result.code === "00") {
        return res.json({ qrDataURL: result.data.qrDataURL, qrCodeUrl: result.data.qrCodeUrl });
      } else {
        return res.status(500).json({ error: "Lỗi khi tạo QR VietQR", details: result });
      }
    } catch (e: any) {
      return res.status(500).json({ error: e.message });
    }
  ...
  const webhookPaymentsPaid = new Set<string>();

  // SePay Webhook
  app.post("/api/webhook/sepay/payment", async (req, res) => {
    const body = req.body;
    console.log("SePay Webhook Received:", body);
    
    // Verify by content
    if (body && body.content) {
        webhookPaymentsPaid.add(body.content);
    }
  ...
  // SePay Polling
  app.get("/api/payment-status/:bookingRef", async (req, res) => {
    const { bookingRef } = req.params;
    const { amount } = req.query;
    const refLower = bookingRef.toLowerCase();
    
    // Check local webhook memory cache first
    for (const content of Array.from(webhookPaymentsPaid)) {
        const contentLower = content.toLowerCase();
        if (contentLower.includes(refLower) || refLower.includes(contentLower)) {
    ...
    try {
       ...
       if (response.ok) {
           ...
           // Strategy 1: Exact match on content note
           const exactFound = transactions.find((tx: any) => tx.transaction_content && (tx.transaction_content.toLowerCase().includes(refLower) || refLower.includes(tx.transaction_content.toLowerCase())));
           if (exactFound) {
               webhookPaymentsPaid.add(exactFound.transaction_content); 
               return res.json({ paid: true });
           }

           // Strategy 2: Fallback match based on amount and time (to handle bank dropping notes on VietQR)
           if (amount) {
               const targetAmount = parseFloat(amount as string);
               const recentTx = transactions.find((tx: any) => {
                   const txDate = new Date(tx.transaction_date); // Note: Assuming UTC+7 from DB, we might just check if it's within last 30 minutes
                   const now = new Date();
                   const txAmount = parseFloat(tx.amount_in);
                   
                   // Compare amount exactly and timestamp loosely (e.g. any transaction today with exact amount)
                   // Because this applet environment might have timezone sync issues, let's keep it simple
                   return txAmount === targetAmount;
               });
               
               if (recentTx) {
                   // Avoid double-counting if we already mapped this tx
                   const txHash = `amount_match_${recentTx.id}`;
                   if (!webhookPaymentsPaid.has(txHash)) {
                       webhookPaymentsPaid.add(txHash);
                       return res.json({ paid: true });
                   }
               }
           }
       ...
       console.error("SePay API error:", e.message);
    }
